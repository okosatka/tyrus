 ...
 */
public class AlarmTest extends TestContainer {

    public AlarmTest() {
        setContextPath("/sample-alarm");
    }

    @Test
    public void testEcho() throws DeploymentException, InterruptedException, IOException {
        final Server server = startServer(AlarmEndpoint.class);

        final CountDownLatch tickerLatch = new CountDownLatch(3);
        ...
        try {
            ...
            }, ClientEndpointConfig.Builder.create().build(), URI.create("ws://localhost:8080/sample-alarm/alarm"));

            assertTrue(onOpenLatch.await(1, TimeUnit.SECONDS));

            //there should be 3 ticks in 11sec (at 0s, 5s and 10s)
            assertTrue(tickerLatch.await(15, TimeUnit.SECONDS));
